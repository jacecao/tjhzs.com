  ...
  class newsModel {
    ...
    public function get_image_news ($max=6) {
      ...
      if (count($allNews) > $max) {
        $allNews = array_slice($allNews, 0, $max-1);
      }
      // 将新闻中储存images的id的字符串转换为id数组
      for ($i = 0; $i < count($allNews); $i++) {
        $allNews[$i] = $this -> reset_imgs_id($allNews[$i]);
        // 根据poster的id值获取图片
        $allNews[$i]['poster'] = $this -> get_img_data($allNews[$i]['poster']);
        // 根据imges的id值获取图片
        $allNews[$i]['images'] = $this -> get_img_data($allNews[$i]['images']);
      }
      return $allNews;
    }
